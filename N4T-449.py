...
#This test is for Automating N4T-449:
#User should be able to verify the sub-field "Substation Name" under Labels and Interfaces option in the Web Page of T/Guard-408XT Device
...
if Common.verifyText("Test_Login.Login.Connection Successful?", "Login Page exists?", "Username:", "Login page not loaded!") != 1:
		...
		time.sleep(0.5)
		Substation_name = browser_driver.find_element_by_id("system_lbl_2")
		Substation_name.clear()
		Substation_name.send_keys("IDC Ahmedabad")
		...
		time.sleep(0.5)
		Substation_Name = browser_driver.find_element_by_id("system-info-2").get_attribute('innerHTML')
		Common.verifyEqual("Test_Labels.Labels.Substation_name?","Substation_name Exists?","IDC Ahmedabad",Substation_Name,"Names do not exist in this page")
		
		time.sleep(1)
		browser_driver.get(Common.Status_address)
		time.sleep(0.5)
		Substation_Name = browser_driver.find_element_by_id("si_info_2").get_attribute('innerHTML')
		Common.verifyEqual("Test_Labels.Labels.Substation_name?","Substation_name Exists?","IDC Ahmedabad",Substation_Name,"Names do not exist in this page")
